...
## **Abstract**
Partial Differential Equations (PDEs) describe the laws that govern systems everywhere from stellar mergers to the volatility of financial markets. This project bridges those extremes by applying machine learning to accelerate PDE-based modeling in computational physics and quantitative finance. I began by developing traditional numerical solvers using Runge–Kutta 4 methods as baselines. I then engineered a Physics-Informed Neural Network (PINN) to approximate solutions to the Black–Scholes PDE, a foundational equation for valuing European-style options, integrating the equation, initial and boundary conditions, and financial constraints into the loss function. The PINN achieved option pricing results within 0.1% of high-resolution numerical methods while running over 72× faster. Finally, I scaled the model to the more complex Heston PDE, a higher-dimensional, stochastic extension of Black–Scholes that is not easily solvable in closed form and cannot be efficiently handled by standard numerical methods without extensive transformations. These results underscore the potential for physics-based deep learning techniques to power faster, more accurate financial modeling, bringing tools from computational physics to the trading floor.

---

## **About**
As of summer 2025, I am a rising sophomore at Drexel University triple-majoring in Physics, Economics, and Mathematics. This summer I was accepted into Drexel's **Students Tackling Advanced Research** or (**STAR Scholars**) program. I was lucky enough to meet Drexel Physics' very own Dr. Niharika Sravan in my Computational Physics I course, and she agreed to be my mentor. She is an expert in Physics-based machine learning and computational physics as a whole. She introduced me to machine learning and used Physics-Informed Neural Networks (PINNs) as a way to dip my toes in the water. She also allowed me to explore the machine learning space by using PINNs in computational finance. As a physicist, I love challenges and puzzles, and believe me, there are puzzles everywhere. Puzzles I've previously tried to solve have ranged from Philadelphia's housing discrimination/displacement crisis to how to consistently win no limit hold 'em.However, the puzzle that has spoken to me since I can remember has been the market. Now, do I think it truly is solvable? Maybe not. Does this project solve the market? No, but it was a stepping stone to better understand how it works and the tools we have at our disposal to understand it.
...
### **Week 7** - _Finishing our Project_
This week, I finished the project! It was more or less straight-forward. I adapted the Black-Scholes PINN for the Heston and it was very successful. I struggled with the hyperparameter tuning, but I managed to optimize it very well with a loss below 0.01. A new technique I had to deploy was to actually hardcode the terminal condition (t=0 --> V=0), since the model was struggling to learn it through standard means. Overall, I'm proud of what we've made and I hope to find some new use cases for PINNs.
<table align="center">
    <tr>
        <td align="center">
            <img src="./media/poster.png" alt="Scientific Poster of our Results" height="600"><br>
            <em> <br>Our poster for the STAR showcase!<br></em>
        </td>
    </tr>
</table>